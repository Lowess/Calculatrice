...
void MainWindow::slotConnection()
{
    ...
    //Connexion des boutons operateurs classiques + - * /
    ...
    //Connexion des boutons operations speciales POW MOD SIGN
    ...
    //Connexion des operateurs mathematiques
    //Sin Cos Tan
    ...
    //Connexion des operateurs de pile Swap Sum Mean Clear Drop Dup
    ...
    //Connection du mode degrees / radians
...
void MainWindow::enterPressed(){
    if(ui->lineEdit->text()=="")
        ...
        try{
            ...
            LogSystem::ecrireLog(LogMessage("Erreur impossible d'effectuer l'operation demandee classe mainWindow ",ERREUR));
            QMessageBox msgBox;
            msgBox.setText(e.what());
            msgBox.exec();
        }
...
void MainWindow::memorisePileUndo(){
    //Memorise la pile pour permettre le undo
    Gardien* g=Calculatrice::getInstance().get_gardien();
    //Memorise la pile courante
...
void MainWindow::save(){
    ...
    file.close();
    LogSystem::ecrireLog(LogMessage("Sauvegarde de contexte effectuee classe mainWindow ",INFO));
...
void MainWindow::load(){
    ...
    //On re-insert les elements sauvegardes dans la pile
    QString buf;
    while(!in.atEnd()){
        in >> buf;
    ...
    LogSystem::ecrireLog(LogMessage("Restauration de contexte effectuee classe mainWindow ",INFO));
}
