...
//Implementation des methodes de la class "Complexe"
...
LO21::Complexe::Complexe(const LO21::Constante& a,const LO21::Constante& b){
    ...
    if(tmpA == 0 || tmpB == 0){
        LogSystem::ecrireLog(LogMessage("Erreur classe complexe ",ERREUR));
        throw CalculatriceException(typeid(this).name(), OTHER, "Creation de complexe impossible : pas de nombre pour instancier");
    }
    _a = tmpA;
    _b = tmpB;
        ...
        if (tmp_e == 0) {
            ...
            if (tmp_re == 0) {
                ...
                if(tmp._a == 0 && tmp._b == 0){
                    LogSystem::ecrireLog(LogMessage("Erreur division par zero classe complexe ",ERREUR));
                    throw CalculatriceException(typeid(this).name(), MATHS, "Complexe::Divison : essai de division par 0 (vous voulez vraiment detruire l'univers?)!");
                }
