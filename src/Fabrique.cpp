...
void LO21::Fabrique::creer(const QString& text) const{
    ...
    for(it=result.begin(); it!=result.end(); ++it){ //On parcours notre expression otee des espaces
        ...
        switch (getTypeSousChaine(*it)){
            ...
            case RATIONNEL:{
                QString tmp(*it);
                QStringList tmpl=tmp.split("/"); //Separation num / den
            ...
            case COMPLEXE:{
                if(!Option::getInstance().get_complexe()){
                    throw CalculatriceException(typeid(this).name(),OPTION,"Le mode complexe n'est pas active, autorisez le mode complexe dans les options pour utiliser ce type de constante");
                }
                QString tmp(*it);
                QStringList tmpl=tmp.split("$"); //Separation Re $ Im
                creer(tmpl.value(0));
                creer(tmpl.value(1));
                //Expression* im=p->pop();
                //Expression* re=p->pop();
